...
async fn start_iproxy(udid: &str, port: u16) -> Result<tokio::process::Child> {
...
/// Ensure the agent is running and reachable at `localhost:port`, auto-starting it (port
/// forward + `xcodebuild test-without-building`) if it isn't. Returns `true` if the
/// agent ends up reachable, `false` if it couldn't be started (caller should fall back
/// to WDA/idb - this must never be the only path).
