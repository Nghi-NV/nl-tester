...
import { useFileStore, useEditorStore, useExecutionStore, useEnvStore, getAllDescendantFiles } from '../stores';
import { FileNode } from '../types';
import {
  Folder, FolderOpen, FileCode, Plus, Trash2, ChevronRight, ChevronDown, Play, Loader2, Square, Pencil
} from 'lucide-react';
  ...
  return (
    <div className="select-none">
      ...
      >
        ...
        <span className={clsx("mr-2", isNodeRunning ? "text-amber-400" : "text-cyan-500/80")}>
          {isNodeRunning ? (
      ...
      for (const file of filesToRun) {
        ...
        try {
          const result = await runTestFlow(
            ...
            file.id,
            file.name,
            () => { }, // No live steps update for batch to keep it performant
            signal
          );
  ...
  return (
    <div className="w-64 h-full bg-slate-950 border-r border-borderGlass flex flex-col">
      <div className="p-4 border-b border-borderGlass flex items-center justify-between">
        ...
        <button
          onClick={() => addFile(null, 'folder', 'New Project')}
